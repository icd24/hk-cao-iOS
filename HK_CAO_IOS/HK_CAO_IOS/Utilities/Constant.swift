//
//  constant.swift
//  HK_CAO_IOS
//
//  Created by HieuNV on 26/06/2024.
//

import SwiftUI

struct AppColors {
    static let main = Color("main")
    static let mainWhite = Color("mainWhite")
    static let main300 = Color("main300")
    static let textWhite = Color("textWhite")
    static let textDark = Color("textDark")
    static let backgroundWhite = Color("backgroundWhite")
    static let backgroundRegister = Color("backgroundRegister")
    static let backgroundTermOfService = Color("backgroundTermOfService")
    static let transparent = Color("transparent")
    static let registerDescriptionBorder = Color("registerDescriptionBorder")
    static let tutorialAuction = Color("tutorialAuction")
    
    static let textFieldTitle = Color("textFieldTitle")
    static let inputText = Color("inputText")
    static let hyperLink = Color("hyperLink")
    
    static let tabAccent = main
    static let tabUnselect = Color("tabUnselect")
    static let tabUnselectUI = UIColor(named: "tabUnselect")
    static let maskColor = Color("mask")
    static let shadowRound = Color("shadowRound")
    static let tabBackground = main

    static let shadow30 = Color("shadow30")
    static let shadow10 = Color("shadow10")
    static let alertBackground = maskColor
}
